...
#### 応用：サブエージェント連携と異種モデル議論による精度向上

sub-agent は単独で使うより、**役割を分けた複数 sub-agent を連携**させることで真価を発揮する。さらに Claude 以外のモデル（Gemini / GPT など）を sub-agent として組み込み、**異種モデル同士に同じ問いへ答えさせて指摘を突き合わせる**ことで、出力精度は単一モデル比で目安 10 ポイント前後向上するとされる（arxiv 2502.08788 等の 2025 年研究を踏まえた目安）。

**連携パターンの使い分け**

Anthropic 公式 "Building Effective Agents" が整理する 4 パターンに、マルチモデル前提の「議論型」を加えて整理する。なお **議論型（Multi-Agent Debate、以下 MAD）** とは、複数 LLM が主張と反論を複数ラウンド重ねて合意形成するフレームワークを指す。

| パターン | 代表ユースケース | 強み | コスト目安 |
|---|---|---|---|
| パイプライン型（逐次） | 調査 → 執筆 → レビュー | 段階ごとの品質保証 | 低 |
| 並列レビュー型 | コードレビュー多角化・設計レビュー | 視点の網羅・独立性担保 | 中 |
| オーケストレータ-ワーカー型 | 複雑タスクの動的分解 | 事前に分解できない課題に対応 | 中〜高 |
| 議論型（Multi-Agent Debate） | 難問の設計判断・ファクトチェック | 相互反論による誤り訂正 | 高 |
| 評価-最適化ループ型 | 品質基準を満たすまでの反復改善 | 自動収束 | 中 |

> **ポイント**：2025 年の再評価研究（arxiv 2502.08788 など）では、議論型の精度向上の大部分は「議論」ではなく**異種モデルのアンサンブル効果**に由来すると報告されている。実務ではまず **並列レビュー型 × 異種モデル** を検討し、それでも精度不足のときだけ議論型を追加するのが費用対効果的に優位。

**異種モデル並列レビュー構成**

同じ問いを Claude / Gemini / GPT の異種 3 モデルに並列で解かせ、指摘の重なり方で優先度を決める構成。
...
- **Claude Code（メイン）**：タスクを分配し、結果を集約する司令塔。
- **異種 sub-agent**：各モデルは独立したコンテキストで同一の問いに回答。発信元を伏せて（匿名化）集約することで、強いモデルへの同調バイアスを抑える。
- **集約ルール**：2 モデル以上が同じ論点を指摘した項目は「高優先度」、1 モデルのみの指摘は「minority view」として人間が判断する。多数決だけで切り捨てない運用が要点。

**実装例：Gemini CLI を sub-agent として呼ぶ**

`.claude/agents/gemini-reviewer.md` に以下を配置し、Bash 経由で Gemini CLI を呼び出す。

```markdown
---
name: gemini-reviewer
description: Gemini の大規模コンテキスト（Gemini 2.x 系で目安 1M トークン以上。設定モデルに依存）を活かしてリポジトリ横断レビューを行います
...
2. `gemini -p "以下のコードを設計観点・性能観点・セキュリティ観点でレビューせよ..."` を Bash で実行
3. 出力を「重大度 / 箇所（file:line）/ 根拠 / 推奨対応」の4列で整理して返す
4. 自分（Gemini 経由）以外のエージェントの意見は参照しない（独立性担保）
```

**実装例：omni-ai-mcp 経由で GPT を呼ぶ**

`omni-ai-mcp`（PyPI のコミュニティ製パッケージ。採用前にメンテ状況を必ず確認）のような MCP サーバーを `.mcp.json` に登録すると、OpenRouter 経由で GPT / Gemini / 400 以上のモデルを sub-agent のツールとして利用できる。
...
sub-agent 側では `tools: mcp__omni-ai__chat` のように MCP ツールを指定し、同一プロンプトを異種モデルへ送信する。

**運用プロンプト例（議論ラウンド 1 回・多数決集約）**
...
> **💡 コスト・品質のトレードオフ（2025 年研究を踏まえた推奨）**
> - **議論より異種アンサンブル優先**：同種モデルのみの議論は独立性が低く効果が薄い。異種モデルの並列出力を多数決するほうが目安で費用対効果が高い（GSM-8K 検証の一例、目安：同種 82% 程度 → 異種 91% 程度。arxiv 2502.08788 参照）。
> - **議論ラウンドは 1 回を基本**：ラウンドを増やすと発散・同調バイアスでかえって精度が下がる場合がある。収束しなければ人間に委ねる。
> - **ルーティングで分ける**：日常タスクは単一モデル（Sonnet 等）、重要判断・セキュリティレビュー・設計分岐のみ 3 モデル並列にする。常時並列は LLM 呼び出しが 3 倍以上になりコストが急増する。
> - **minority-veto を採用**：多数決のみで集約すると少数派の鋭い指摘が埋没する。1 件のみの指摘も必ず人間レビューに回す運用を徹底する。
> - **2026 年 4 月時点で Agent Teams 機能**（Claude Code の実験的機能）を使えばチームメイト同士の直接通信も可能になるが、まずはメイン経由の集約型から始めるのが安全。エージェント定義の基本は `4.3 Skills・Hooks・Subagents の設計`、モデル選択の一覧は `1.8 モデル別の得意領域と使い分け`、ツール横断のメモリ共有は `12.9 マルチエージェント協調時の中央メモリ` を参照。
