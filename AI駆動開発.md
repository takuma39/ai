...
| --------------- | ------------------------ | -------------------------------------- | --------------------------- | ------------------------------- |
| 要件定義        | `SPEC.md`                | 受け入れ基準と非機能要件が記載済み     | 曖昧な目標のまま実装開始    | 仕様書の連携管理（Notion）      |
| 設計            | `CLAUDE.md` / ルール定義 | 変更方針と制約が合意済み               | 既存制約を無視した設計      | 既存スキーマ調査（DB MCP）      |
| 実装            | 実装PR                   | テスト成功 + 仕様準拠                  | 1セッションに複数課題を混在 | タスク取得（GitHub/Jira）       |
| レビュー/テスト | レビュー記録、テスト結果 | 人間承認 + 自動テスト通過              | AIレビューのみでマージ      | 自動テスト生成（Playwright）    |
| CI/CD           | 監視・運用設定           | デプロイ後監視とロールバック手順を確認 | 自動化だけ導入し監視未整備  | インシデント調査（Datadog等）   |
