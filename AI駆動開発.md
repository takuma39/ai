...
- コピペしてすぐ使えるチーム標準プロンプト集は「8.2 チーム標準プロンプトテンプレート」を参照

> **※バージョン表記について**：本資料に記載されているモデル名（Claude 4.6系、GPT-5、o4等）やツールバージョンは、**2026年4月時点の最新メジャーバージョン**を想定しています。お使いの環境のバージョンと差異がある場合は、各ツールの公式ドキュメントで最新情報を確認してください。
...
| 規模             | 推奨セット                                                   | 採用基準                                             |
| ---------------- | ------------------------------------------------------------ | ---------------------------------------------------- |
| 個人開発         | Claude Code + **Cursor** or **VS Code + Copilot** + GitHub   | 学習コストを抑え、実装とレビューの往復を高速化したい |
| 小〜中規模チーム | 上記 + Playwright                                            | PR品質ゲートと回帰テストを定常運用したい             |
| 企業運用         | 上記 + AIゲートウェイ + 監査ログ基盤                         | 機密管理、監査、コスト統制を組織要件として満たしたい |

> **IDE選択の注意：CursorとGitHub Copilotは排他的に選ぶ**
> CursorとGitHub Copilotを同時にオンにすると、インライン補完（Ghost Text）が競合してUXが著しく低下する。IDE選択は以下のどちらか一方を推奨する：
> - **Cursor単体**：補完＋エージェント機能（Composer）を内包。追加プラグイン不要
> - **VS Code + GitHub Copilot**：既存のVS Code環境にCopilotを追加。Copilot Edits でエージェント的な編集も可能
>
...
graph TD
    ...
    subgraph cli["CLIレイヤー（ターミナルでの自律的ビルド・テスト・一括処理）"]
        CC["Claude Code"]
    end
...
| テスト          | テスト結果 + 視覚差分レポート | 自動テスト全件通過 + UI 視覚差分が許容範囲内 | E2Eテストの未整備           | Playwright MCP、Chrome MCP、Figma MCP（視覚一致検証） |
...
| **GPT系（GPT-5 / o4系）** | 構造化ドキュメント整形・網羅的な観点出し       | SPEC.md のレビュー、抜け漏れチェック、目次構造の整備   |
...
本章では、UI/UXデザインプロセスにおけるAI活用を3段階に分けて解説します。

```mermaid
flowchart LR
    A["3.1 AIでラフ案を生成"] --> B["3.2 AIでデザインを評価"]
    B --> C["3.3 Figma MCP → コード実装"]
    C --> D["3.4 デザイントークン管理"]
...
2024年11月にAnthropicが発表、2025年3月にLinux Foundation傘下に移管。AI業界のデファクトスタンダードとなった。
...
解決策として、SQLite などの軽量 DB を MCP 経由で「中央メモリ」として共有する構成が有効。

```mermaid
flowchart TB
    CC["Claude Code"] --> Mem["中央メモリ<br/>SQLite (MCP)"]
    Mem --> CC
    Cur["Cursor"] --> Mem
    Mem --> Cur
    Dev["Devin"] --> Mem
