...
基本設計で定めたアーキテクチャ・データモデル・インターフェースを、実装可能なレベルまで詳細化するフェーズ。クラス設計・モジュール設計・シーケンス図・処理フロー設計が主な成果物となる。AI開発環境の設定（CLAUDE.md・Skills・Hooks）は 4章 AI環境設定で扱う。
...
### 6.5 MCP（Model Context Protocol）エコシステム
...
### 6.7 プロジェクト管理 MCP連携（GitHub / Jira）
...
### 6.9 Apidog MCP × Claude Code で仕様-実装ギャップを埋める
...
### 6.10 Context7 MCP で最新ライブラリドキュメントを参照
...
### 6.11 Serena MCP で大規模コードベースのトークン消費を抑える
