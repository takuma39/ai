...
> **コスト注意**：Writer/Reviewer パターンは2つの独立したセッションを使うため、トークン消費が実質2倍になる。複雑な機能や品質が特に重要なコードに絞って適用し、単純な修正には通常の1セッションで対応するのが経済的。
...
| 6   | テスト駆動開発の強制                 | Hook や CLAUDE.md で「実装前にテスト作成」を必須化         |
