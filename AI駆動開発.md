...
小規模チームが最短で成果を出すための初期構成。まずは以下3点のみをMUSTとして開始し、残りはSHOULD/MAYで段階導入する。

| 優先度 | 施策 | 目的 | 完了条件 |
| --- | --- | --- | --- |
| MUST | `SPEC.md` を作成し、要件をBDDで定義 | 実装の手戻り削減 | 受け入れ基準（Given/When/Then）がそろっている |
| MUST | AI実装後にテスト実行 + Diffレビュー | 品質事故の抑制 | テスト成功、差分確認済み |
| MUST | PRに人間レビューを必須化 | 誤実装・過信の防止 | 承認者1名以上でマージ |
| SHOULD | `.claude/CLAUDE.md` で規約共有 | 生成品質の安定化 | 主要ルールが文書化されている |
| MAY | Playwright/CIの自動化 | 回帰検知の強化 | 主要フローが自動テスト化されている |

**切り戻し基準（運用ガードレール）**
- AI支援でバグ密度が増加した場合: 対象タスクを一時的に手動実装へ戻す
- レビュー負荷が過大な場合: AI適用範囲を「テスト作成・リファクタ」に限定する
- コストが予算超過した場合: 高コストモデル利用をSHOULDからMAYへ格下げする
...
| フェーズ | 成果物 | 完了条件 | よくある失敗 |
| --- | --- | --- | --- |
| 要件定義 | `SPEC.md` | 受け入れ基準と非機能要件が記載済み | 曖昧な目標のまま実装開始 |
| 設計 | `CLAUDE.md` / ルール定義 | 変更方針と制約が合意済み | 既存制約を無視した設計 |
| 実装 | 実装PR | テスト成功 + 仕様準拠 | 1セッションに複数課題を混在 |
| レビュー/テスト | レビュー記録、テスト結果 | 人間承認 + 自動テスト通過 | AIレビューのみでマージ |
| CI/CD | 監視・運用設定 | デプロイ後監視とロールバック手順を確認 | 自動化だけ導入し監視未整備 |
