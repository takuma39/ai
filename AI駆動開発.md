...
- 最短で導入する場合は「最小導入セット」→「1. 要件定義」→「4. 実装」を先に読む
- チーム全体へ展開する場合は「10. チームワークフロー」まで通読する
- RAGやAIゲートウェイなどの高度な構成は、導入後の拡張として段階的に採用する
...
| データベース（MCP連携）| Postgres MCP / SQLite MCP                     | AIによるスキーマ参照・クエリ実行の支援                     |
...
| 規模             | 推奨セット                           | 採用基準                                             |
| ---------------- | ------------------------------------ | ---------------------------------------------------- |
| 個人開発         | Claude Code + Cursor + GitHub        | 学習コストを抑え、実装とレビューの往復を高速化したい |
| 小〜中規模チーム | 上記 + Copilotレビュー + Playwright  | PR品質ゲートと回帰テストを定常運用したい             |
| 企業運用         | 上記 + AIゲートウェイ + 監査ログ基盤 | 機密管理、監査、コスト統制を組織要件として満たしたい |
...
flowchart TD
    ...
    F -->|デプロイ| G["7. 本番運用・監視<br/>(Datadog / Sentry)"]
...
| フェーズ        | 成果物                   | 完了条件                               | よくある失敗                | 主なMCP用途                           |
| --------------- | ------------------------ | -------------------------------------- | --------------------------- | ------------------------------------- |
| 要件定義        | `SPEC.md`                | 受け入れ基準と非機能要件が記載済み     | 曖昧な目標のまま実装開始    | 仕様書の連携管理（Notion MCP）        |
| 設計            | `CLAUDE.md` / ルール定義 | 変更方針と制約が合意済み               | 既存制約を無視した設計      | 既存スキーマ調査（DB MCP）            |
| 実装            | 実装PR                   | テスト成功 + 仕様準拠                  | 1セッションに複数課題を混在 | タスク取得（GitHub/Jira MCP）、DB MCP |
| レビュー        | レビュー記録             | Copilot + Claude Code + 人間承認       | AIレビューのみでマージ      | GitHub MCP（PR差分取得）              |
| テスト          | テスト結果               | 自動テスト全件通過                     | E2Eテストの未整備           | Playwright MCP、Chrome MCP            |
| CI/CD           | デプロイ完了             | デプロイ成功 + ロールバック手順を確認  | 自動化だけ導入し監視未整備  | ―                                     |
| 運用・監視      | インシデント対応記録     | アラート設定 + 対応フロー確認済み      | 監視未設定でリリース        | Datadog/Sentry MCP、Slack MCP         |
...
Cursorによる開発では、複数ファイルにまたがるコード生成とターミナル操作を統合した「Composer」を中心としたエージェントワークフローを標準とします。

#### モデル選択戦略

| モデル             | 用途                                             |
| ------------------ | ------------------------------------------------ |
| Claude Sonnet 4 系 | 日常的なコーディング・複数ファイルを含む機能実装 |
| o3 / Opus 系       | 複雑なアーキテクチャ設計・難解なバグ修正         |
| Haiku 系           | 高速な補完・単純なリファクタリング・文言修正     |
...
最新の GitHub Copilot（v2）では、Claude 3.5 Sonnet や OpenAI o3 といった強力なモデルが選択可能になりました。これにより、Cursorと同等の「プロジェクト全体（Workspace）を把握し、複数ファイルを一括で自律編集するエージェント機能（Copilot Edits等）」が利用可能になっています。
...
| 選択モデル        | 用途・得意領域                                                             |
| ----------------- | -------------------------------------------------------------------------- |
| Claude 3.5 Sonnet | 新規機能の実装、フロントエンドコンポーネント・UIの生成（現在もっとも推奨） |
...
### 4.8 データベース MCP連携（Postgres / SQLite）
...
sequenceDiagram
    ...
    participant DB as Postgres/SQLite (MCP)
...
### 7.7 モニタリング・インシデント対応 MCP連携（Datadog / Sentry）
...
sequenceDiagram
    ...
    participant DB as Datadog (MCP)
    participant Code as コードベース
    
    人間->>CC: "Sentryの最新のCriticalエラーを調査して"
    CC->>Sentry: 最近のエラーログとスタックトレースを取得
    Sentry-->>CC: "NullReferenceException in paymentFlow.ts line 42"
    CC->>Code: paymentFlow.ts の該当行を読み込み
    CC->>DB: エラー発生時刻前後の関連APIメトリクスを取得
    DB-->>CC: 関連する他サービスAPIからの500エラー急増を確認
    Note over CC: エラー内容、DBログ、コードを総合して分析
