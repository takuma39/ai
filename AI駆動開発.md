...
|---|---|---|---|---|---|
| **Context7 MCP** | ドキュメント | Upstash | ライブラリ・フレームワークの最新公式ドキュメントをオンデマンド取得。`resolve-library-id` でライブラリを特定し、`query-docs` で現行APIシグネチャ・サンプルコードを返す | 知識カットオフ以降に変更されたライブラリ（React 19 / Next.js 15 / Tailwind 4 等）の正確なコード生成 | 5.10 |
| **Serena MCP** | コード検索・編集 | oraios | LSP を仲介し、`find_symbol` / `find_referencing_symbols` / `replace_symbol_body` 等でシンボル単位のコード取得・編集を提供 | 大規模モノレポでのトークン消費削減（最大70%程度）。500行超のファイルが多いプロジェクトで特に有効 | 5.11 |
| **Figma MCP** | デザイン | Figma（公式） | Figmaデザインからコンポーネント仕様・デザイントークンを取得。Code Connectによるコードベースとのコンポーネント紐付け | デザイン→コード変換、視覚一致検証ループ（Playwright MCPと併用） | 4.5 |
| **Apidog MCP** | API仕様 | Apidog | OpenAPI仕様をClaude Codeから直接参照。AIによるSchema自動生成・準拠性チェック・テストケース自動生成 | API仕様をSSoT（Single Source of Truth）として、仕様-実装ギャップの検知と修正PR自動生成 | 1.7, 5.9 |
| **Playwright MCP** | テスト・ブラウザ | Microsoft | ブラウザ自動操作・E2Eテストシナリオ生成・スクリーンショット差分検出。Playwright Agents（v1.56〜）で自律テストも可能 | E2Eテスト自動化、仕様書からの操作手順変換、Figma MCPとの視覚一致検証 | 7.2, 7.7 |
| **GitHub MCP** | プロジェクト管理 | GitHub | Issue・PR・コメント・差分情報の参照・操作。CI/CDステータスの取得 | Issue駆動の自律開発フロー、PRレビュー時の差分取得、タスク自動取得 | 5.7 |
| **Jira MCP** | プロジェクト管理 | Atlassian | チケット情報の参照・ステータス更新。スプリント・エピック単位での検索 | タスクの自動取得・優先度判定、実装完了後のステータス自動更新 | 5.7 |
| **Postgres MCP** | データベース | コミュニティ | スキーマ一覧・テーブル定義の取得、読み取り専用クエリの実行 | 正確なカラム名・型に基づくSQL/ORM生成、マイグレーション作成時の既存スキーマ調査 | 5.8 |
...
| モック生成            | Figma AI（Make Designs）/ v0.dev（Vercel）    | 要件定義フェーズのモックファースト開発。仮SPEC.mdからUIモックを高速生成し、ステークホルダーとの反復レビューで仕様を確定する |
...
```mermaid
flowchart TD
    A["1. 要件定義・仕様書作成<br/>(Spec-Driven Development)"] --> B["2. 基本設計<br/>(アーキテクチャ・DB・API・画面)"]
    B --> B2["3. 詳細設計<br/>(CLAUDE.md / Skills / クラス設計)"]
    B2 --> TS["タスク分割<br/>(Claude Code + Jira / GitHub MCP)"]
    TS --> C["4. UI/UXデザイン<br/>(Claude Code + Figma MCP)"]
    C --> D["5. 実装<br/>(Claude Code主体 / IDEは微修正・レビュー)"]
    D --> T["7. テスト作成・実行<br/>(Playwright / Vitest)"]
    T -->|"失敗"| D
    T -->|"成功"| E["6. コードレビュー<br/>(Copilot + Claude Code + 人間)"]
    E -->|修正| D
    E -->|承認| F["8. CI/CD<br/>(GitHub Actions)"]
...
| --------------- | ------------------------ | -------------------------------------- | --------------------------- | ------------------------------------- |
| 1. 要件定義     | `SPEC.md`                | 受け入れ基準と非機能要件が記載済み     | 曖昧な目標のまま設計開始    | 仕様書の連携管理（Notion MCP）、API仕様連携（Apidog MCP） |
| 2. 基本設計     | システム構成図・ER図・API仕様書・画面遷移図 | 各設計書のAIレビュー完了、ステークホルダー合意済み | 設計書なしで詳細設計に着手 / AIに丸投げで人間レビューなし | DB MCP（既存スキーマ調査）、Apidog MCP（API設計） |
| 3. 詳細設計     | `CLAUDE.md` / ルール定義・クラス図・シーケンス図 | AI開発環境の設定完了、変更方針と制約が合意済み | 既存制約を無視した設計 / コンテキスト管理戦略の未策定 | DB MCP（スキーマ詳細）    |
| (3) タスク分割  | タスクチケット一覧（Jira / GitHub Issues） | 全タスクにストーリーポイント・優先度・依存関係が設定済み | 粒度が粗すぎて見積もり不能 / 依存関係の未整理 | Jira MCP、GitHub MCP（チケット自動生成） |
| 4. UI/UXデザイン| Figma デザインファイル + デザイントークン（tokens.json） | Figma MCP連携済み + デザイントークンがコードに反映済み | スペックなしにFigmaで作り込み開始 | Figma MCP、Framelink MCP              |
| 5. 実装         | 実装PR                   | テスト成功 + 仕様準拠（Apidog MCP 導入時は API仕様差分が解消済み） | 1セッションに複数課題を混在 | タスク取得（GitHub/Jira MCP）、DB MCP、Apidog MCP（仕様差分検知） |
| 7. テスト       | テスト結果 + 視覚差分レポート | 自動テスト全件通過 + UI 視覚差分が許容範囲内 | E2Eテストの未整備           | Playwright MCP、Figma MCP（視覚一致検証） |
| 6. レビュー     | レビュー記録             | Copilot + Claude Code + 人間承認       | AIレビューのみでマージ      | GitHub MCP（PR差分取得）              |
| 8. CI/CD        | デプロイ完了             | デプロイ成功 + ロールバック手順を確認  | 自動化だけ導入し監視未整備  | ―                                     |
| 8. 運用・監視   | インシデント対応記録     | アラート設定 + 対応フロー確認済み      | 監視未設定でリリース        | Datadog MCP、Slack MCP                |
...
| ステップ | 担当 | アクション |
|---|---|---|
| モック生成 | Figma AI / v0.dev | 仮SPEC.mdからモックを複数パターン生成 |
...
> **Figma MCP との違い**：Figma MCP は「既存のFigmaデザインをコードに変換する」ツール。モック生成は Figma AI が担い、Figma MCP はその後の実装フェーズで活用する（詳細は 4.5 参照）。
...
```mermaid
flowchart LR
    A["要件定義<br/>SPEC.md"] --> B["基本設計<br/>アーキテクチャ・DB・API"]
    B --> C["詳細設計<br/>クラス設計・AI環境設定"]
    C --> D["実装"]
```
...
| 成果物 | 内容 | AI活用ポイント |
|---|---|---|
| システム構成図 | 全体アーキテクチャ（C4モデル等） | Mermaidで構成図を生成させる |
| ER図 | テーブル設計・リレーション | erDiagram で叩き台を自動生成 |
| API仕様書 | エンドポイント・リクエスト/レスポンス | Apidog AIでSchema自動生成 |
| 画面遷移図 | 画面間の遷移とトリガー | stateDiagramで自動生成 |
| 非機能要件定義書 | 性能・セキュリティ・可用性 | 要件からの定量化をAIに支援させる |
...
基本設計で定めたアーキテクチャ・データモデル・インターフェースを、実装可能なレベルまで詳細化するフェーズ。AI駆動開発では、従来のクラス設計・シーケンス図に加えて「AI開発環境の設計」（CLAUDE.md・Skills・Hooks等）が重要な成果物となる。
...
| AI環境 | — | CLAUDE.md・Skills・Hooks・Subagents |

#### 詳細設計の成果物一覧

| 成果物 | 内容 | AI活用ポイント |
|---|---|---|
| CLAUDE.md / ルール定義 | AI開発環境の設定 | プロジェクト制約をAIに正確に伝える |
| Skills / Hooks / Subagents | AI自動化の設計 | 繰り返しタスクの自動化パターン |
| クラス図 | モジュール構造・依存関係 | Mermaid classDiagramで叩き台生成 |
| シーケンス図 | 処理フロー・API間連携 | ユースケースから自動生成 |
| コンポーネント設計 | UI部品の責務分割 | ディレクトリ構造の提案 |

### 3.2 AI開発環境の設計

#### CLAUDE.md の設計（最重要ファイル）
...
### 3.3 Skills・Hooks・Subagents の設計
...
### 4.3 AIエージェントによるUX自動テストとペルソナ評価（ベストプラクティス）
...
### 4.4 AIによる定量UX・行動データ解析（Analytics SaaS連携）
...
> **コスト注意（FinOps観点）**：Writer/Reviewer パターンは2つの独立したセッションを使うため、トークン消費が実質2倍になる。**コアロジックやセキュリティが重要なファイルに限定して使用**し、単純な修正やUI調整には通常の1セッションで完結させるのが経済的。LLMコストの全体管理については「10.6 AIゲートウェイ」を参照。

### 5.2 Cursor IDE (v2) ベストプラクティス
...
### 5.3 GitHub Copilot (v2) ベストプラクティス
...
### 5.5 MCP（Model Context Protocol）エコシステム
...
### 5.7 プロジェクト管理 MCP連携（GitHub / Jira）
...
### 5.9 Apidog MCP × Claude Code で仕様-実装ギャップを埋める
...
### 5.10 Context7 MCP で最新ライブラリドキュメントを参照
...
### 5.11 Serena MCP で大規模コードベースのトークン消費を抑える
...
### 7.7 Figma MCP × Playwright MCP の視覚一致検証ループ

生成コードとデザインの視覚的ズレを、AI エージェントだけで検出・修正するループ。従来の VRT（7.6）がピクセル差分を「検出」する役割なら、このループは Figma の設計意図と照合して **「修正まで自動化」** する点が新しい。
...
### 8.2 Self-Healing Pipeline パターン
...
### 8.5 LLM FinOpsとコスト最適化（プロンプトキャッシュ等）
...
#### 8.6.5 RAG導入の最大の壁：アクセス権限（パーミッション）管理
...
### 8.7 モニタリング・インシデント対応 MCP連携（Datadog）
...
### 8.8 ChatOpsと人間参加型（Human-in-the-Loop）インシデント対応
...
| 実装       | `Implement [feature] in @[file]. Follow patterns in @[example]. Run tests after implementing.`   | 5.1            |
| テスト     | `Write failing tests for [feature], then implement to make them pass.`                           | 5.6, 7.3       |
| レビュー   | `Review this PR against @SPEC.md. Check for security, performance, and spec compliance.`         | 6.3            |
| リファクタ | `Refactor @[file] to improve [concern]. Preserve all behavior - run tests before and after.`     | 5.1            |
...
### 9.5 Skills × MCP の連携フロー

`9.4` で「MCP は減らせ」と述べたが、一方で **Context7・Serena・Apidog** など追加で価値を生む MCP も存在する。両立のカギは「**どの MCP をいつ呼ぶか**を Skill 層に記述し、AI の迷いを減らす」設計にある。
...
### 10.2 OWASP Top 10 for LLMs 2025
...
### 10.5 ローカルLLM / SLM（小規模言語モデル）の選択肢
...
### 10.6 AIゲートウェイによるLLM APIセキュリティ対策
...
### 10.9 Claude Code セキュリティ初期設定の見直し
