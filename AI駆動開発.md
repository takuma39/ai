...
> **※AIゲートウェイの適用範囲**：ゲートウェイが保護できるのは「自社アプリ/コードからLLM APIを呼ぶ」経路のみ。v0.dev・Galileo AI・GitHub Copilotなどの外部SaaSはそれぞれが直接LLMを呼んでいるため、ゲートウェイでは介入できない。
...
flowchart TD
    ...
    B --> C["3. UI/UXデザイン<br/>(Figma MCP / v0.dev)"]
...
| **v0.dev (Vercel)** | UIとコードを同時生成、Vibe Coding向け | Figmaと疎結合 | デザイナー不在時の管理画面、使い捨て検証用 |
| **Lovable / bolt.new** | UI + バックエンド + DBを含むフルスタックアプリをブラウザ上で生成 | 複雑なカスタマイズには不向き | MVP・社内ツールの爆速構築 |
| **Locofy.ai** | Figmaから本番品質コードへの変換 | AI生成ではなく変換ツール | フルプロジェクトの設計→コード引き渡し |
| **Figma + MCP** | デザインシステムとの完全統合 | 初期設定が複雑 | 正規プロダクション開発全体（メインフロー） |

**推奨ワークフロー（デザイナーがチームにいる環境の王道）**：

基本的に、正規の開発フローにおいて **「Figma（ラフ〜正規デザイン） → Figma MCP → 実装（Cursor/Claude等）」** の直結フローが王道（正解）です。

```
Figma（デザイナーが作成・整えたデザインシステム）
    ↓
Figma MCP経由でAI（Claude/Cursor）に読み込ませる
    ↓
AIがピクセルパーフェクトなコードを生成
```

> **⚠️ アンチパターン：正規フローへの v0.dev の過度な介入**
> v0.dev や Galileo AI は非常に強力ですが、デザイナーがいるチームでの役割は「デザイナー不在時の管理画面作成」や「要件定義直後の使い捨てプロトタイプ作成（触り心地の検証用）」に限定すべきです。
> 正規のデザインフロー（Figma中心）の途中に無理にv0を挟むと、デザイントークン（色や余白のルール）が乖離し、後から整合性を合わせるための「手戻り（負債）」が大量に発生します。
...
v0.devやGalileo AIでUIを自動生成するワークフローでは、「意図したデザイン変更」と「予期せぬレイアウト崩れ」の切り分けが課題になる。AIベースのVRTツールは、ピクセル完全一致ではなく文脈で判断する。
...
| デザイン生成 | v0.dev / Galileo AI | **× 適用不可** | 外部SaaSが内部で直接LLMを呼ぶため介入不可 |
...
> **まとめ**：ゲートウェイの主戦場は「**実装フェーズ（自社アプリ→LLM）**」と「**Claude/AIのAPI利用（プロキシ経由）**」。v0.dev等の外部SaaSツールにはそれぞれのサービス側のセキュリティ機能に依存する。
...
| v0 Enterprise | エンタープライズプラン | データ保持ポリシーを確認 |
