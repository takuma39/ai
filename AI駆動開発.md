...
| IaC静的解析 | Checkov / k8sgpt / Trivy | Terraform / k8sマニフェスト / コンテナイメージのセキュリティスキャン |
| インフラ管理 | Terraform MCP / Docker MCP | IaCコードの生成・コンテナイメージのビルド・レジストリ管理 |
| デプロイ | GitHub Actions | Staging → Production への段階的デプロイ |
| ロールバック | GitHub Actions | テスト失敗・静的解析違反・デプロイ失敗時の自動ロールバック |
